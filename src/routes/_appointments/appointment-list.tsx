...
function TarifsMedicauxPage() {
  ...
  // Catégories et départements disponibles
  const categories = [
    "Consultation",
    "Imagerie",
    "Biologie",
  ...
  // Ouvrir le dialogue pour un nouveau service
  ...
  // Ouvrir le dialogue pour modifier un service
  ...
  const saveService = () => {
    ...
    if (currentService.id) {
      // Modification
      setServices(
        services.map((svc) => (svc.id === currentService.id ? newService : svc))
      );
    } else {
      // Nouveau service
  ...
  // Formater le prix en devise locale
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      {/* Dialogue de gestion des services */}
        ...
        <DialogContent className="sm:max-w-[600px]">
          ...
          <DialogFooter>
            <Button type="button" onClick={saveService}>
              {currentService.id ? "Enregistrer" : "Ajouter le service"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
